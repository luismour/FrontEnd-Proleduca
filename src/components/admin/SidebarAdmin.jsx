...
// --- Componentes de Ícones SVG (Exemplos) ---
// Você pode criar arquivos separados para estes ícones ou usar uma biblioteca como react-icons
...
const InstitutionsIcon = () => (
  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5 mr-3 group-hover:text-white transition-colors">
    {/* Ícone de "prédio" ou "biblioteca" seria bom aqui. Usando um genérico por enquanto. */}
...
const ScholarshipHoldersIcon = () => ( // Ícone para Bolsistas (Exemplo: Graduação)
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5 mr-3 group-hover:text-white transition-colors">
...
const InscriptionsIcon = () => ( // Ícone para Inscrições (Exemplo: Documento com check)
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5 mr-3 group-hover:text-white transition-colors">
...
// Adicione um ícone para Dependentes se necessário
...
const NavItem = ({ to, icon, children }) => {
  ...
  const activeClasses = "bg-blue-700 text-white shadow-inner"; // Estilo aprimorado para link ativo
  const inactiveClasses = "text-gray-300 hover:bg-gray-700 hover:text-white";

  return (
    <NavLink
      to={to}
      end // Adiciona 'end' para que o NavLink só seja ativo se a rota for exata (útil para /admin/dashboard e /admin)
      className={({ isActive }) => 
...
export default function SidebarAdmin() {
  return (
    <aside className="w-64 h-screen bg-slate-800 text-white flex flex-col fixed top-0 left-0 z-40 shadow-lg">
      {/* Logo ou Título do Painel */}
      <div className="px-4 h-16 flex items-center border-b border-slate-700"> {/* Altura fixa para o header da sidebar */}
        {/* Exemplo de Logo: Substitua pelo seu */}
        {/* <img src="/path-to-your-admin-logo.png" alt="Logo Admin" className="h-8 w-auto mr-3"/> */}
        <h2 className="text-lg font-semibold text-white truncate">Painel Administrativo</h2>
      </div>

      {/* Navegação com rolagem interna */}
      <nav className="flex-grow px-3 py-4 space-y-1.5 overflow-y-auto"> {/* Ajustado padding e space */}
